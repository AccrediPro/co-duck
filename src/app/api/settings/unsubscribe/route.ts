...
export async function GET(request: Request) {
  ...
  try {
    ...
    return new Response(
      htmlPage(
        'Unsubscribed',
        `You have been unsubscribed from ${categoryLabels[category]}. You can re-enable this in your <a href="${process.env.NEXT_PUBLIC_APP_URL || 'https://co-duck.com'}/dashboard/settings">account settings</a>.`
      ),
      {
        status: 200,
        headers: { 'Content-Type': 'text/html' },
